<?php
	...
	if($server == "localhost:8080") {
		...
		$caminho_raiz = $_SERVER['DOCUMENT_ROOT']."/ferraz";
		$caminho_site = "http://".$_SERVER['SERVER_NAME'].":8080/ferraz";
		$caminho_plugins = "http://".$_SERVER['SERVER_NAME'].":8080/plugins/";
	} else if($server == "localhost") {
		
		$caminho_raiz = $_SERVER['DOCUMENT_ROOT'].'/ferraz';
		$caminho_site = "http://".$_SERVER['SERVER_NAME'].'/ferraz';
		$caminho_plugins = "http://".$_SERVER['SERVER_NAME'].'/plugins/';
	} else if($server == "comberweb.com.br") {
		
		$caminho_raiz = $_SERVER['DOCUMENT_ROOT']."/projetos/ferraz";
		$caminho_site = "http://".$_SERVER['SERVER_NAME'].'/projetos/ferraz';
	...
	if($server == "localhost" || $server == "localhost:8080") {
		...
		if($_SERVER['SERVER_NAME'] == 'localhost' || $_SERVER['SERVER_NAME']=='127.0.0.1' || $urlArr[0] == 'ferraz') {
		    array_shift($urlArr);
		}
	} else if($server == "comberweb.com.br") {
		...
		if($urlArr[0]=='projetos' && $urlArr[1]=='ferraz'){
		    array_shift($urlArr);
		    array_shift($urlArr);
		}
	}
